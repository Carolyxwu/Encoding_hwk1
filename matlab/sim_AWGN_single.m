...
%         random_bits = (rand([1, N_info_bits])>0.5); % Generate info bits. 
        ...
        wave_transmit(n0_iter,:) = syms2waveform(syms_transmit, waveform_conf, disp_waveform);
        RF_signals_trans(sim_iter, :) = wave_transmit;  % RECORD
        
        [syms_recv, wave_recv(n0_iter,:)] = waveform2syms(wave_transmit, n0_arr_encoding(n0_iter), length(syms_transmit), ...
...
time_elapsed = toc;
save(['data/sim_AWGN_single_', strrep(datestr(datetime), ':', '_'), '.mat']);